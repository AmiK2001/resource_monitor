...
object CpuInfo {
    ...
    /*
       ...
       */
    val cpuUsageSinceLastCall: Int
        get() = if (Build.VERSION.SDK_INT < 26) getCpuUsage(coresUsageDeprecated) else 0
    ...
    val coresUsageGuessFromFreq: IntArray
        get() {
            initCoresFreq()
            val nbCores: Int = mCoresFreq.size + 1
            val coresUsage = IntArray(nbCores)
            coresUsage[0] = 0
            for (i in mCoresFreq.indices) {
                coresUsage[i + 1] = mCoresFreq.get(i.toInt()).getCurUsage()
    ...
    fun initCoresFreq() {
        if (mCoresFreq == null) {
            val nbCores = nbCores
            mCoresFreq = java.util.ArrayList<CoreFreq>()
            for (i in 0 until nbCores) {
                mCoresFreq.add(CoreFreq(i.toInt()))
            }
        }
    }
...
//Check if filename is "cpu", followed by one or more digits//Private Class to display only CPU devices in the directory listing
        ...
        get() {
            ...
            class CpuFilter : java.io.FileFilter {
                override fun accept(pathname: File): Boolean {
                    ...
                    return if (java.util.regex.Pattern.matches("cpu[0-9]+", pathname.getName())) {
                        true
                    } else false
                }
            }
    ...
    private var mCoresFreq: java.util.ArrayList<CoreFreq>? = null// compute usage
...
// check for strange values// cpu lines are only at the top of the file
     ...
     */
    @get:kotlin.jvm.Synchronized
    val coresUsageDeprecated: IntArray
        get() {
            val numCores = nbCores + 1 // +1 for global cpu stat

            // ensure mPrevCores list is big enough
            if (mPrevCoreStats == null) mPrevCoreStats = java.util.ArrayList<CoreStat>()
            while (mPrevCoreStats.size < numCores) mPrevCoreStats.add(null) //new CpuStat(-1, -1));

            // init cpuStats
            val coreStats: java.util.ArrayList<CoreStat> = java.util.ArrayList<CoreStat>()
            ...
            try {
                ...
                try {
                    ...
                    for (i in 0 until numCores) {
                        ...
                        // try get core stat number i
                        curCoreStat = readCoreStat(i.toInt(), line)
                        if (curCoreStat != null) {
                            val prevCoreStat: CoreStat = mPrevCoreStats.get(i.toInt())
                            ...
                            // cur becomes prev (only if cpu online)
                            mPrevCoreStats.set(i.toInt(), curCoreStat)
    ...
    private fun readCoreStat(coreNum: Int, line: String): CoreStat? {
        ...
        try {
            val cpuStr: String
            cpuStr = if (coreNum > 0) "cpu" + (coreNum - 1) + " " else "cpu "
            if (line.contains(cpuStr)) {
                val toks: Array<String> = line.split(" +".toRegex()).dropLastWhile { it.isEmpty() }
                    .toTypedArray()
                ...
                val active: Long = toks[1].toLong() + toks[2].toLong() + toks[3].toLong()
                val total: Long =
    ...
    // previous stat read
    private var mPrevCoreStats: java.util.ArrayList<CoreStat>? = null

    private class CoreFreq internal constructor(var num: Int) {
        var cur = 0
        var min = 0
        var max = 0
...
//                else
    ...
    private class CoreStat internal constructor(var active: Float, var total: Float)
}